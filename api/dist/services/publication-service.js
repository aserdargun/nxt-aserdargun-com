...
export class PublicationService {
    ...
    async publish(input) {
        ...
        if (initialCausality.predecessorIdentity !== causality.predecessorIdentity)
    ...
    async ensureOperationFolder(operation, input) {
        const desired = {
            kind: input.kind,
            state: "prepared",
        ...
        if (activeOperation.createIntent === null) {
            activeOperation = await this.updateOperation(activeOperation, (current) => ({ ...current, createIntent: desired }), input.context);
        }
        ...
        const owned = existing.filter((file) => markerMatches(file, input.marker, storageKind, operation.publicId, operationId));
        ...
        if (existing.length !== 0)
            throw new ApiResponseError("CONFLICT");
        if (activeOperation.createIntent?.state !== "prepared")
            throw new ApiResponseError("DRIVE_UNAVAILABLE");
        ...
        if (createdId !== undefined && candidate.id !== createdId)
            throw new ApiResponseError("CONFLICT");
        if (!markerMatches(candidate, input.marker, storageKind, operation.publicId, operationId))
            throw new ApiResponseError("CONFLICT");
    ...
    async cancelCreateIntent(operation, context) {
        if (operation.createIntent === null)
            return operation;
        return this.updateOperation(operation, (current) => ({ ...current, createIntent: null }), context);
    }
    async completeCreateIntent(operation, folder, context) {
        const intent = operation.createIntent;
        if (intent === null ||
    ...
    async updateOperation(operation, transform, context) {
        ...
        try {
            const committed = await this.options.manifestStore.compareAndSet((manifest) => {
                ...
                return bumpManifest(manifest, {
                    operations: manifest.operations.map((candidate) => candidate.operationId === operation.operationId ? proposed : candidate)
                });
            }, { context });
            ...
            if (result !== undefined && proposed !== undefined && sameOperationState(result, proposed))
                return result;
            throw error;
        }
    }
    async abandonOperation(operationId, publicId, orphans) {
        ...
        await this.options.manifestStore.compareAndSet((manifest) => {
            ...
            if (createIntent !== null && createIntent !== undefined && createIntent.state !== "prepared") {
                if (createIntent.state === "recoverable")
                    return manifest;
                return bumpManifest(manifest, {
                    operations: manifest.operations.map((candidate) => candidate.operationId === operationId
                        ? { ...candidate, createIntent: { ...createIntent, state: "recoverable" } }
                        : candidate)
                });
            }
    ...
    async recoverCreateIntents(context) {
        const snapshot = await this.options.manifestStore.read(context);
        const observedAt = this.now().toISOString();
        const candidates = snapshot.value.operations.filter((operation) => operation.createIntent !== null && (operation.createIntent.state === "recoverable" ||
        ...
        const selected = Array.from({ length: Math.min(MAX_CREATE_RECOVERIES_PER_REQUEST, candidates.length) }, (_value, index) => candidates[(start + index) % candidates.length]);
        ...
        await this.options.manifestStore.compareAndSet((manifest) => {
            const remaining = manifest.operations.filter((operation) => operation.createIntent !== null && (operation.createIntent.state === "recoverable" ||
                operation.createIntent.state === "attempted" && operationCanBeRecovered(operation, observedAt))).length;
    ...
    async recoverCreateIntent(operation, context) {
        ...
        if (intent.kind === "public-root") {
            if (current.cleanupSlots !== 2 || current.publicFolderId !== null || current.publicFolderVersion !== null ||
            ...
            if (current.publicFolderId === null || current.publicFolderVersion === null ||
            ...
            if (publicFolder.version !== current.publicFolderVersion ||
                !folderMatches(publicFolder, publishedRoot.id, current.publicId, publicationMarker(current.publicId, "public"), "public", current.publicId) || publicFolder.appProperties?.nxtPublicationOperation !== undefined ||
        ...
        if (!markerMatches(listed, intent.marker, storageKind, current.publicId, expectedOperationId) ||
            (intent.folderId !== null && listed.id !== intent.folderId))
            return;
        const folder = await this.options.storage.get(listed.id, context);
        if (!folderMatches(folder, intent.parentFolderId, intent.folderName, intent.marker, storageKind, current.publicId, expectedOperationId) || folder.appProperties?.nxtPublicationAssetId !== undefined ||
    ...
    async queueRecoveredCreate(operation, folder, publicFolder, context) {
        const intent = operation.createIntent;
        if (intent === null)
            return;
        const artifactCount = intent.kind === "revision" && operation.cleanupSlots === 2 ? 2 : 1;
        const cleanupCandidates = Array.from({ length: artifactCount * MAX_RANDOM_ID_COLLISIONS }, () => this.id());
        const queuedAt = this.now().toISOString();
        let proposedCleanup;
        try {
            await this.options.manifestStore.compareAndSet((manifest) => {
                ...
                const artifacts = intent.kind === "public-root"
                    ? [publicRootCleanupArtifact(operation.publicId, folder, this.options.publishedRootId)]
                    : [
                        revisionCleanupArtifact(operation.publicId, operation.operationId, intent.folderName, intent.marker, folder, intent.parentFolderId),
                ...
                if (artifacts.length !== artifactCount || artifacts.length > operation.cleanupSlots)
                    throw new ApiResponseError("CONFLICT");
                const usedIds = manifestIdentifierSet(manifest);
                const cleanupIds = takeUniqueIdentifiers(cleanupCandidates, artifacts.length, usedIds);
                if (cleanupIds.length !== artifacts.length)
            ...
            const recovered = await this.options.manifestStore.read(context).catch(() => undefined);
            if (recovered !== undefined && proposedCleanup !== undefined &&
...
const manifestReferencesFolder = (manifest, folderId) => manifest.entries.some((entry) => entry.publicFolderId === folderId || entry.revisions.some((revision) => revision.snapshotFolderId === folderId || revision.assetsFolderId === folderId)) || manifest.tombstones.some((tombstone) => tombstone.publicFolderId === folderId) ||
    manifest.operations.some((operation) => operation.publicFolderId === folderId || operation.revisionFolderId === folderId || operation.createIntent?.folderId === folderId);
...
const sameCreateIntentTarget = (left, right) => left.kind === right.kind && left.parentFolderId === right.parentFolderId && left.folderName === right.folderName &&

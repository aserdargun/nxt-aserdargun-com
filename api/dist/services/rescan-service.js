    ...
    async completeScan(state, transition, priorCursor, context) {
        ...
        try {
            await this.options.indexStore.compareAndSet((index) => {
                ...
                const fingerprints = new Map(state.conflictMutationFingerprints.map((conflict) => [conflict.id, conflict.fingerprint]));
                if (index.generation !== state.baseGeneration ||
                    ...
                    index.pendingMutations.some((mutation) => mutation.phase !== "conflicted" || fingerprints.get(mutation.id) !== mutationFingerprint(mutation)))
                    throw new ApiResponseError("CONFLICT");

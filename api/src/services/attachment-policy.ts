...
const validWebp = (bytes: Uint8Array): boolean => {
  ...
  if (ascii(bytes, 12, 4) !== "VP8 ") return false;
...
const validVp8 = (bytes: Uint8Array): boolean => {
  ...
  if (!keyFrame || version > 3 || !showFrame || firstPartitionSize === 0 || !equal(bytes, 3, [0x9d, 0x01, 0x2a])) return false;
...
type PdfValue =
  ...
  | { kind: "dictionary"; entries: Map<string, PdfValue> }
  | { kind: "array"; items: PdfValue[] }
  | { kind: "string" }
  | { kind: "boolean"; value: boolean }
  | { kind: "null" };

type PdfXrefEntry = { objectNumber: number; offset: number; generation: number; active: boolean };

const validPdf = (bytes: Uint8Array): boolean => {
  if (bytes.length < 64 || bytes.length > MAX_ATTACHMENT_BYTES || ascii(bytes, 0, 5) !== "%PDF-") return false;
  ...
  if (!Number.isSafeInteger(xrefOffset) || xrefOffset < header[0].length || xrefOffset >= text.length || text.slice(xrefOffset, xrefOffset + 4) !== "xref") return false;

  const parsedXref = parseClassicXref(text, xrefOffset);
  if (parsedXref === undefined || parsedXref.startxref !== xrefOffset || parsedXref.end !== text.length) return false;
  const { entries, trailer } = parsedXref;
  if (entries.size < 2 || entries.size > MAX_PDF_XREF_ENTRIES) return false;
  const size = trailer.entries.get("Size");
  const root = trailer.entries.get("Root");
  if (
    size?.kind !== "number" || !size.integer || size.value < 2 || size.value > MAX_PDF_XREF_ENTRIES ||
    root?.kind !== "reference" || trailer.entries.has("Prev") || trailer.entries.has("XRefStm")
  ) return false;
  const highestObjectNumber = Math.max(...entries.keys());
  if (size.value !== highestObjectNumber + 1) return false;

  const zero = entries.get(0);
  if (zero === undefined || zero.active || zero.offset !== 0 || zero.generation !== 65_535) return false;
  for (const entry of entries.values()) {
    if (entry.objectNumber < 0 || entry.objectNumber >= size.value) return false;
    if (entry.active) {
      if (entry.objectNumber === 0 || entry.generation >= 65_535 || entry.offset < header[0].length || entry.offset >= xrefOffset) return false;
  ...
  if (active.length === 0 || active.length > MAX_PDF_XREF_ENTRIES - 1) return false;
  const objects = new Map<number, { generation: number; value: PdfValue }>();
  let regionCursor = header[0].length;
  for (let index = 0; index < active.length; index += 1) {
    const entry = active[index] as PdfXrefEntry;
    const limit = active[index + 1]?.offset ?? xrefOffset;
    if (entry.offset < regionCursor || limit <= entry.offset || !onlyPdfTrivia(text, regionCursor, entry.offset)) return false;
    const parsed = parseIndirectObject(text, entry, limit);
    if (parsed === undefined || parsed.end > limit || objects.has(entry.objectNumber)) return false;
    objects.set(entry.objectNumber, { generation: entry.generation, value: parsed.value });
    regionCursor = parsed.end;
  }
  if (!onlyPdfTrivia(text, regionCursor, xrefOffset) || objects.size !== active.length) return false;

  const rootObject = objects.get(root.objectNumber);
  if (rootObject === undefined || rootObject.generation !== root.generation || rootObject.value.kind !== "dictionary") return false;
  const rootType = rootObject.value.entries.get("Type");
  if (rootType?.kind !== "name" || rootType.value !== "Catalog") return false;
  return [...objects.values(), { generation: 0, value: trailer }].every(({ value }) =>
    pdfReferences(value).every((reference) => {
...
const parseClassicXref = (text: string, xrefOffset: number): { entries: Map<number, PdfXrefEntry>; trailer: Extract<PdfValue, { kind: "dictionary" }>; startxref: number; end: number } | undefined => {
  ...
  while (entries.size <= MAX_PDF_XREF_ENTRIES) {
    ...
    if (text.startsWith("trailer", cursor) && pdfDelimited(text, cursor + 7)) break;
    ...
    if (subsection === null) return undefined;
    const first = Number(subsection[1]);
    const count = Number(subsection[2]);
    if (!Number.isSafeInteger(first) || !Number.isSafeInteger(count) || first < 0 || count < 1 || first + count > MAX_PDF_XREF_ENTRIES || entries.size + count > MAX_PDF_XREF_ENTRIES) return undefined;
    ...
    for (let index = 0; index < count; index += 1) {
      ...
      if (line === null || entries.has(objectNumber)) return undefined;
  ...
  if (!text.startsWith("trailer", cursor) || !pdfDelimited(text, cursor + 7)) return undefined;
  const parser = new PdfParser(text, cursor + 7, text.length);
  const trailer = parser.parseValue();
  if (trailer?.kind !== "dictionary") return undefined;
  parser.skipTrivia();
  if (!parser.consumeKeyword("startxref") || !isPdfWhitespace(text.charCodeAt(parser.position))) return undefined;
  parser.skipWhitespace();
  const start = parser.position;
  while (parser.position < text.length && isAsciiDigit(text.charCodeAt(parser.position))) parser.position += 1;
  if (parser.position === start) return undefined;
  const startxref = Number(text.slice(start, parser.position));
  if (!Number.isSafeInteger(startxref) || !isPdfWhitespace(text.charCodeAt(parser.position))) return undefined;
  parser.skipWhitespace();
  if (!text.startsWith("%%EOF", parser.position)) return undefined;
  parser.position += 5;
  parser.skipWhitespace();
  return parser.position === text.length ? { entries, trailer, startxref, end: parser.position } : undefined;
};

const parseIndirectObject = (text: string, entry: PdfXrefEntry, limit: number): { value: PdfValue; end: number } | undefined => {
  ...
  parser.skipTrivia();
  if (!parser.consumeKeyword("endobj")) return undefined;
...
class PdfParser {
  ...
  public constructor(private readonly text: string, public position: number, private readonly limit: number) {}

  public parseValue(depth = 0): PdfValue | undefined {
    if (depth > MAX_PDF_PARSE_DEPTH || this.tokens >= MAX_PDF_TOKENS) return undefined;
    this.tokens += 1;
    this.skipTrivia();
    if (this.position >= this.limit) return undefined;
    if (this.text.startsWith("<<", this.position)) return this.parseDictionary(depth + 1);
    const character = this.text[this.position];
    if (character === "[") return this.parseArray(depth + 1);
    if (character === "/") return this.parseName();
    if (character === "(") return this.parseLiteralString();
    if (character === "<") return this.parseHexString();
  ...
  public skipWhitespace(): void {
    while (this.position < this.limit && isPdfWhitespace(this.text.charCodeAt(this.position))) this.position += 1;
  }

  public consumeKeyword(keyword: string): boolean {
    if (!this.text.startsWith(keyword, this.position) || !pdfDelimited(this.text, this.position + keyword.length)) return false;
    this.position += keyword.length;
    return true;
  }

  private parseDictionary(depth: number): PdfValue | undefined {
    this.position += 2;
    ...
    for (;;) {
      ...
      if (key === undefined || entries.has(key.value)) return undefined;
  ...
  private parseArray(depth: number): PdfValue | undefined {
    this.position += 1;
  ...
  private parseName(): Extract<PdfValue, { kind: "name" }> | undefined {
    if (this.text[this.position] !== "/") return undefined;
    this.position += 1;
    const start = this.position;
    while (this.position < this.limit && !isPdfWhitespace(this.text.charCodeAt(this.position)) && !isPdfDelimiter(this.text[this.position])) this.position += 1;
    const raw = this.text.slice(start, this.position);
    if (raw.length === 0 || /#(?![0-9A-Fa-f]{2})/u.test(raw)) return undefined;
    return { kind: "name", value: raw.replace(/#([0-9A-Fa-f]{2})/gu, (_match, value: string) => String.fromCharCode(Number.parseInt(value, 16))) };
  }

  private parseLiteralString(): PdfValue | undefined {
    this.position += 1;
    let depth = 1;
  ...
  private parseHexString(): PdfValue | undefined {
    this.position += 1;
    while (this.position < this.limit) {
      const character = this.text[this.position++];
      if (character === ">") return { kind: "string" };
      const code = character?.charCodeAt(0) ?? -1;
      const hexadecimal = (code >= 48 && code <= 57) || (code >= 65 && code <= 70) || (code >= 97 && code <= 102);
      if (!hexadecimal && !isPdfWhitespace(code)) return undefined;
    }
    return undefined;
  }

  private parseNumberOrReference(): PdfValue | undefined {
    const first = this.readRegularToken();
    if (first === undefined || !/^[+-]?(?:\d+(?:\.\d*)?|\.\d+)$/u.test(first)) return undefined;
    ...
    if (integer && value >= 0 && Number.isSafeInteger(value)) {
      ...
      if (second !== undefined && /^\d+$/u.test(second)) {
        ...
        this.skipTrivia();
        if (generation <= 65_535 && this.consumeKeyword("R")) return { kind: "reference", objectNumber: value, generation };
      }
  ...
  private readRegularToken(): string | undefined {
    const start = this.position;
    while (this.position < this.limit && !isPdfWhitespace(this.text.charCodeAt(this.position)) && !isPdfDelimiter(this.text[this.position])) this.position += 1;
...
const pdfReferences = (value: PdfValue): Array<{ objectNumber: number; generation: number }> => {
  if (value.kind === "reference") return [{ objectNumber: value.objectNumber, generation: value.generation }];
  if (value.kind === "array") return value.items.flatMap(pdfReferences);
  if (value.kind === "dictionary") return [...value.entries.values()].flatMap(pdfReferences);
  return [];
};

const onlyPdfTrivia = (text: string, start: number, end: number): boolean => start <= end && skipPdfTrivia(text, start, end) === end;

const skipPdfTrivia = (text: string, initial: number, limit: number): number => {
  let cursor = initial;
  while (cursor < limit) {
...
const pdfLineEnd = (text: string, start: number): { contentEnd: number; next: number } | undefined => {
  for (let cursor = start; cursor < text.length; cursor += 1) {
    if (text[cursor] === "\n") return { contentEnd: cursor, next: cursor + 1 };
    if (text[cursor] === "\r") return { contentEnd: cursor, next: text[cursor + 1] === "\n" ? cursor + 2 : cursor + 1 };
  }
  return undefined;
};

const consumePdfLineEnd = (text: string, cursor: number): number => {
  if (text[cursor] === "\n") return cursor + 1;
  if (text[cursor] === "\r") return text[cursor + 1] === "\n" ? cursor + 2 : cursor + 1;
  return -1;
};

const pdfDelimited = (text: string, cursor: number): boolean => cursor >= text.length || isPdfWhitespace(text.charCodeAt(cursor)) || isPdfDelimiter(text[cursor]);
const isPdfWhitespace = (code: number): boolean => code === 0 || code === 9 || code === 10 || code === 12 || code === 13 || code === 32;
const isPdfDelimiter = (character: string | undefined): boolean => character !== undefined && "()<>[]{}/%".includes(character);

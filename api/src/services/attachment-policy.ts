...
const skipGifSubBlocks = (bytes: Uint8Array, initial: number): number | false => {
  let offset = initial;
  while (offset < bytes.length) {
...
const validPdf = (bytes: Uint8Array): boolean => {
  if (bytes.length < 32 || ascii(bytes, 0, 5) !== "%PDF-") return false;
  const text = new TextDecoder("latin1").decode(bytes);
  if (!/^%PDF-[12]\.[0-9]/u.test(text) || !/\b(?:xref|\/Type\s*\/XRef)\b/u.test(text) || !/\btrailer\b/u.test(text) || !/\bstartxref\s+\d+\s+%%EOF\s*$/u.test(text)) return false;

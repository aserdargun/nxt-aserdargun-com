...
The deployment workflow reads only `AZURE_STATIC_WEB_APPS_API_TOKEN_SWA_NXT_ASERDARGUN_COM`. GitHub permissions are `contents: read`; no OIDC/id-token permission or `github_id_token` input exists. Deployment is serialized with cancellation disabled, waits for its own portable validation, and independently refuses any ref other than `refs/heads/main`, including manual dispatches. Real macOS acceptance runs in the separate CI workflow and must be reviewed alongside the deployment result.
...
Publishing copies an immutable source snapshot and only referenced, verified attachments into the private publication area. The anonymous API resolves an opaque public ID through the allowlisted manifest, returns generic redacted not-found responses, and never exposes owner/Drive/snapshot identifiers. Revocation removes manifest reachability. Public note metadata is projected from the verified snapshot manifest without downloading attachment bodies; an allowlisted asset alone proceeds to ancestry, checksum, and MIME verification. Stable manifest versions share one parsed body per warm Functions instance, a changed Drive version forces a fresh verified read, and a 16 MiB serialized manifest ceiling is checked from metadata before media download.

Anonymous note and asset work also shares one per-instance rolling gate: at most eight requests execute concurrently and 120 requests are admitted per 60 seconds. Saturation returns the same generic not-found shape before Drive-backed service resolution, preserving owner-route capacity. This application-level gate is defense in depth, not a distributed quota across Azure scale-out; production should retain platform/edge throttling appropriate to the expected sharing volume. The public route imports no owner shell and sends no private/session request.
...
Local verification does not access or mutate live Drive, GitHub, Azure, DNS, remotes, workflows, secrets, or deployments. Live operations require a fresh exact target display and action-time authorization.

Normal application releases preserve and read-only verify the existing `nxt.aserdargun.com` mapping. Stop before any Azure custom-hostname, `_dnsauth` TXT, CNAME/A/AAAA/ALIAS, IHS/e-destek, certificate, nameserver, apex, `www`, or mail mutation. Those actions require the separate domain workflow and a new confirmation at mutation time.

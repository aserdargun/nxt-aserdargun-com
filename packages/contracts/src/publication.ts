...
export const PublishedAssetNameSchema = AttachmentNameSchema.refine((value) =>
  value === value.normalize("NFC") && [...value].every((character) => {
...
export const PublicationRevisionSchema = z.object({
  ...
  value.assets.forEach((asset, index) => {
    if (ids.has(asset.assetId)) context.addIssue({ code: "custom", path: ["assets", index, "assetId"], message: "assetId must be unique" });
...
export const PublicationEntrySchema = z.object({
  ...
  if (ids.size !== value.revisions.length) context.addIssue({ code: "custom", path: ["revisions"], message: "revisionId must be unique" });
  if (!ids.has(value.activeRevisionId)) context.addIssue({ code: "custom", path: ["activeRevisionId"], message: "active revision is missing" });
});
...
export const PublicationOperationSchema = z.object({
  ...
  if (intent.kind === "public-root") {
    if (
      value.cleanupSlots !== 2 || value.publicFolderId !== null || value.publicFolderVersion !== null ||
  ...
  if (
    value.publicFolderId === null || value.publicFolderVersion === null ||
    value.revisionFolderId !== null || value.revisionFolderVersion !== null ||
    value.revisionId === null || value.revisionMarker === null ||
...
export const PublicationManifestSchema = z.object({
  ...
  value.entries.forEach((entry, index) => {
    if (publicIds.has(entry.publicId) || noteIds.has(entry.sourceNoteId)) context.addIssue({ code: "custom", path: ["entries", index], message: "active publications must be unique" });
    publicIds.add(entry.publicId);
    noteIds.add(entry.sourceNoteId);
  });
  value.tombstones.forEach((entry, index) => {
    if (publicIds.has(entry.publicId) || noteIds.has(entry.sourceNoteId)) context.addIssue({ code: "custom", path: ["tombstones", index], message: "tombstones must not overlap active entries" });
    publicIds.add(entry.publicId);
    noteIds.add(entry.sourceNoteId);
  });

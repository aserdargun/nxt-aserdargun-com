...
export function deriveIndex(records) {
    const parsed = records.map((record) => ({ record, note: parseNote(record.source) }));
    const seenIds = new Set();
    for (const { note } of parsed) {
        if (seenIds.has(note.frontmatter.id))
            throw new Error(`duplicate note id: ${note.frontmatter.id}`);
    ...
    for (const entry of entries)
        entry.backlinks = unique(backlinks.get(entry.id) ?? []);
